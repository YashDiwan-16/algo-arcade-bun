...
import { GameMilestones } from "@/components/rewards/game-milestones";
...
export default function EndlessRunnerLeaderboardPage() {
  const { data, isLoading } = useQuery({
    queryKey: ["scoreLeaderboard", "endless-runner"],
    queryFn: () =>
          ...
          {data.leaderboard.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="space-y-4">
                <GameMilestones gameId={GAME_CONFIG.gameId} />
              </div>
            </div>
          )}
        </>
      ...
      <Card className="p-8 bg-linear-to-r from-purple-600 via-pink-600 to-blue-600 border-none text-white">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-white/90 max-w-2xl mx-auto">
            Play {GAME_CONFIG.name} now and see if you can make it to the top of
            the leaderboard!
          </p>
          <Link
            href={`/games/${GAME_CONFIG.gameId}`}
